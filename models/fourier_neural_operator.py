...
class _ConditionalFNO(BaseModel, name='FNO'):
    """N-Dimensional Conditional Fourier Neural Operator
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.condition_fno_block = FNOBlocks(
    ...
    def forward(self, x, y, output_shape=None, **kwargs):
        """TFNO's forward pass
        ...
        x = self.lifting(x)
        y = self.condition_lifting(y)

        x_dims = x.shape[2:]
        AdaptPool = getattr(nn, 'AdaptiveAvgPool{}d'.format(len(x_dims)))
        y = AdaptPool(x_dims)(y)
        
        if self.domain_padding is not None:
            x = self.domain_padding.pad(x)
        
        x = torch.cat((x, y), dim=1)
        x = self.condition_fno_block(x, output_shape=output_shape[0])
...
class ConditionalFNO(torch.nn.Module):
    def __init__(
            self, 
            modes, 
            vis_channels, 
    ...
    def forward(self, u, y, output_shape=None, **kwargs):
        batch_size = u.shape[0]
        dims = u.shape[2:]
        posn_emb = make_posn_embed(batch_size, dims).to(u.device)

        u = torch.cat((u, posn_emb), dim=1).float() 
        out = self.model(u, y)
...
class DiscriminatorFNO(torch.nn.Module):
    def __init__(
        ...
        super(DiscriminatorFNO, self).__init__()
        self.model = _ConditionalFNO(
            n_modes=(modes, ) * x_dim, 
            hidden_channels=hidden_channels, 
            in_channels=vis_channels + x_dim, 
    ...
    def forward(self, u, y, output_shape=None, **kwargs):
        ...
        u = torch.cat((u, posn_emb), dim=1).float() 
        out = self.model(u, y)
        # bring the output to a scalar 
        out = out.mean(dim=(2, 3))
        
        return out
            

class TimeConditionalFNO(torch.nn.Module):
    def __init__(
            self, 
            modes, 
            vis_channels, 
        ...
        super(TimeConditionalFNO, self).__init__()
        ...
        self.model = _ConditionalFNO(n_modes=n_modes, 
